...
comma_separator = lambda x, y: f"{str(x)}, {str(y)}"
string_sum = lambda x, y: f"{str(x)} + {str(y)}"
newline_separator = (lambda x, y: str(x) + "\n" + str(y))

def add_includes(data):
    output_buffer = ""
    if (
        "do_not_include_QMK" not in data["parameters"]
        or data["parameters"]["do_not_include_QMK"] != True
    ):
...
def add_parameters(data):
    ...
    output_buffer += f"#define HASH_TYPE {hash_type}" + "\n"
...
def add_keycodes(data):
    ...
    output_buffer = (
        "".join(
            f"#define H_{key} ((HASH_TYPE) 1 << {str(counter)}" + ")\n"
            for key, counter in zip(data["keys"], range(len(data["keys"])))
        )
        + "\n"
    )
    ...
    output_buffer += (
        "    " + reduce(comma_separator, list(data["keys"][1:])) + ",\n"
    )
...
def add_pseudolayers(data):
    ...
    if len(
        [pseudolayer["name"] for pseudolayer in data["pseudolayers"]]
    ) != len({pseudolayer["name"] for pseudolayer in data["pseudolayers"]}):
        raise Exception("The pseudolayers must have unique names")

    pseudolayers = data["pseudolayers"]
    if "ALWAYS_ON" not in [layer["name"] for layer in pseudolayers]:
...
def add_layers(data):
    ...
    for layer, counter in zip(data["layers"], range(len(data["layers"]))):
        if layer["type"] == "auto":
            output_buffer += (
                f"    [{str(counter)}] = "
                + data["parameters"]["layout_function_name"]
                + "("
                + reduce(comma_separator, list(data["keys"]))
                + "),\n"
            )
            ...
            output_buffer += (
                ...
                + data["parameters"]["layout_function_name"]
                + "("
                + reduce(comma_separator, list(layer["keycodes"]))
                + "),\n"
            )
...
def parse_chords(data):
    ...
    for pseudolayer in data["pseudolayers"]:
        ...
        for chord in pseudolayer["chords"]:
            if chord["type"] == "chord_set":
                keycodes = reduce(comma_separator, list(chord["keycodes"]))
            ...
            if chord["type"] == "visual":
                keycodes = reduce(comma_separator, list(chord["chord"]))
                ...
                keycodes = reduce(string_sum, [f"H_{word}" for word in chord["chord"]])
    ...
    keyboard_part_2 += (
        ...
        + reduce(
            comma_separator,
            [f"&chord_{str(i)}" for i in range(number_of_chords)],
        )
        + "\n"
    )
    ...
    keyboard_part_2 += f"#define NUMBER_OF_CHORDS {str(number_of_chords)}" + "\n"
...
def parse_strings_for_chords(data):
    ...
    if len(strings) > 0:
        keyboard_part_1 += "    " + reduce(
            comma_separator, [f"string_{str(i)}" for i in range(len(strings))]
        )
